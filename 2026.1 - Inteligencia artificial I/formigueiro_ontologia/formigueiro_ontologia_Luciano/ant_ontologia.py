...
class FormigaOntologia:
    MAX_PASSOS = 20

    def __init__(self, name, ambiente, formigas_ref, ontology_manager: OntologyManager):
        self.name = name
        self.ambiente = ambiente
        self.formigas_ref = formigas_ref
        self.ontology_manager = ontology_manager
        self.inbox = asyncio.Queue()

        self.position = (0, 0)
        self.route = []
        self.known_food_path = None
    ...
    def _vizinhos_validos(self):
        ...
        Os caminhos inválidos são lidos do ambiente (lista compartilhada),
        que é preenchida via broadcast ou consulta da ontologia ao iniciar.
        """
    ...
    async def _retornar_fracasso(self):
        ...
        self.ontology_manager.registrar_rota(self.name, rota, sucesso=False)
    ...
    async def _retornar_sucesso(self):
        ...
        if self.ambiente.coletar_alimento():
            ...
            # A formiga descobridora memoriza a própria rota
            self.known_food_path = rota

            # Registra na ontologia
            self.ontology_manager.registrar_rota(self.name, rota, sucesso=True)
            self.ontology_manager.incrementar_coleta()

            # Broadcast da rota de sucesso para acelerar convergência
            await self._broadcast({
                'tipo': 'sucesso',
                'de': self.name,
                'rota': rota,
